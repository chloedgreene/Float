    ...
    loop {
        ...
        for path in paths {
            ...
            if final_selection as usize == current as usize {
                ...
                let colour_map =
                    format!("{}/{}", path_raw, v["maps"]["colour_map"].as_str().unwrap());
                let depth_map =
                    format!("{}/{}", path_raw, v["maps"]["depth_map"].as_str().unwrap());

                let cmap = ImageReader::open(colour_map)
                    .unwrap()
                    .with_guessed_format()
                ...
                let dmap = ImageReader::open(depth_map)
                    .unwrap()
                    .with_guessed_format()
                    .expect("Cant find format")
                    .decode()
                ...
                let sky_colour =
                    Color::from_rgba(sky_colour.r, sky_colour.g, sky_colour.b, sky_colour.a);

                return (sky_colour, cmap, dmap);
            }

            draw_text(
                format!("{}{}", g, path.unwrap().path().display()).as_str(),
                25.,
                85. + (25. * current),
                25.,
                WHITE,
